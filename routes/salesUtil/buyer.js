...
const service = require("../../service/salesUtil/BuyerService");
...
router.get('/find', function(req, res, next) {
    ...
    condition = reqUtil.formatCondition(condition);
    let sort = {spellName: 1};

    service
        .find(req.curUser, condition, undefined, sort)
        .then(
        ...
        if (err) {
            ...
            workSheetsFromFile[0].data.forEach(function (data,i) {
                if(i > 0 && data.length > 0){
                    ...
                    if(data[3]){_data['tenant'] = data[3];}
                    if(data[4]){_data['state'] = data[4];}
                    if(data[5]){_data['createTime'] = data[5];}
                    if(data[6]){_data['creater'] = data[6];}
                    if(data[7]){_data['updateTime'] = data[7];}
                    if(data[8]){_data['updater'] = data[8];}
...
router.get('/export', function(req, res, next) {
    ...
    service.find(req.curUser, condition).then(data => {
        ...
        const buffer = xlsx.build([{name: "T_Buyer", data: _data}]);
        let fileName = `/T_Buyer_export${tool.dateFormatter(new Date(), 'yyyyMMdd')}.xlsx`;
