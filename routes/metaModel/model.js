...
const service = require("../../service/metaModel/ModelService");
        ...
        if (err) {
            ...
            workSheetsFromFile[0].data.forEach(function (data,i) {
                if(i > 0 && data.length > 0){
                    ...
                    if(data[3]){_data['attribute'] = data[3];}
...
router.get('/export', function(req, res, next) {
    ...
    service.find(req.curUser, condition).then(data => {
        ...
        const buffer = xlsx.build([{name: "D_Model", data: _data}]);
        let fileName = `/D_Model_export${tool.dateFormatter(new Date(), 'yyyyMMdd')}.xlsx`;
