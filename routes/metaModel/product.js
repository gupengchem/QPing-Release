...
const service = require("../../service/metaModel/ProductService");
...
router.get('/detail/:id', function(req, res, next) {
    ...
    service
        .findObjById(req.curUser, _id, populate)
        .then(
            data => res.send(resUtil.success({data:data})),
            err => res.send(resUtil.error())
        );
        ...
        if (err) {
            ...
            workSheetsFromFile[0].data.forEach(function (data,i) {
                if(i > 0 && data.length > 0){
                    ...
                    if(data[3]){_data['model'] = data[3];}
                    if(data[4]){_data['params'] = data[4];}
...
router.get('/export', function(req, res, next) {
    ...
    service.find(req.curUser, condition).then(data => {
        ...
        const buffer = xlsx.build([{name: "D_Product", data: _data}]);
        let fileName = `/D_Product_export${tool.dateFormatter(new Date(), 'yyyyMMdd')}.xlsx`;
