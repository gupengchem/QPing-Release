...
(function ($) {
    ...
    productGridTemplate.item = function (data) {
        ...
        // console.log(data.priceInfoList && data.priceInfoList[0],priceDef,priceDef && priceDef.budgetValue);
        Dolphin.createImg({
            id: data.code
        }).appendTo(productImg);
        ...
        $('<span class="productPrice">').css('margin-right', '20px').html('单价:' + (priceDef.budgetValue || '-')).appendTo(productInfo2_);
        ...
        if (this.opts.productUnitOption.length > 1) {
            ...
            unitOption = $('<select style="height: 30px;width: 35px;">').appendTo(unitButton);
            for (var i = 0; i < this.opts.productUnitOption.length; i++) {
                if (this.opts.productUnitOption[i].code == 'KAR' || this.opts.productUnitOption[i].code == data.measureUnit){
                    if(this.opts.checkedObj[data[this.opts.idField]] && this.opts.productUnitOption[i].code == this.opts.checkedObj[data[this.opts.idField]].unit){
                        $('<option value="' + this.opts.productUnitOption[i].code + '" selected>').html(this.opts.productUnitOption[i].name).appendTo(unitOption);
                        this.opts.checkedObj[data[this.opts.idField]].saleUnit = this.opts.checkedObj[data[this.opts.idField]].unit;
                    }else{
        ...
        if (this.opts.productUnitOption.length > 1) {
            unitOption.bind('change', function () {
                ...
                thisPriductGrid.changeNum($(thisInput), {unitChange: true,saleUnit:$(this).val()});
            });
        }
