...
(function($) {
    ...
    DOLPHIN.prompt = function(info, param){	//TODO
        ...
        switch(opts.type){
            case 'number':
                input = $('<input type="number" class="form-control" placeholder="'+opts.placeholder+'" value="'+opts.defaultValue+'">');
                break;
            default :
            ...
            switch (thisTool.typeof(method)){
                ...
                case "string":
                    ...
                    for(i = 0; i < methodArray.length; i++){
                        ...
                        if(funcArguments){
                            funcArguments = funcArguments[0].replace(/^\[/, '').replace(/]$/,'').split('|');
                        }else{
                            funcArguments = [];
                        }
            ...
            selectors.each(
                function () {
                    ...
                    var options = $(this).attr('options') || opts.options,
                        ...
                        pleaseSelect = $(this).attr('pleaseSelect') || opts.pleaseSelect,
    ...
    LIST.prototype = {
        ...
        addDataRow : function(data, rowIndex){
            ...
            $.each(this.__columns, function (i, column) {
                ...
                if(column.formatter){
                    col.html(column.formatter.call(thisList, value, data, rowIndex));
                }else{
                    if(thisList.opts.editFlag){
                        var inputItem = null;
    ...
    TREE.prototype = {
        ...
        render : function(nodeList, panel){
            ...
            $.each(nodeList, function(i, node){
                var treeItem = $("<tr>").addClass("treeItem").attr('itemType', node.__type).appendTo(tbody);
                ...
                if(thisTree.opts.checkbox){
                    if(thisTree.opts.onlyLeafCheck && node.__type == 'folder'){
                        $("<td>").css({width:"20px"}).appendTo(treeItem);
                    }else{
                        var treeItemCheckbox = $("<td>").css({width:"20px"}).appendTo(treeItem);
                        var itemCheckbox = $('<input id="' + thisTree.opts.id + '_item_1" name="'+(thisTree.opts.checkboxName || (thisTree.opts.id+'selectedItem'))+'" class="selectedItem" title="'+thisTree.getName(node)+'" style="margin-top: 0px;" >');
                ...
                if(node.__type == 'folder' && node.children){
